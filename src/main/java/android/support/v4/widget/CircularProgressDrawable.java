    ...
    public void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime < 0.5f) {
                ...
                startTrim = (MATERIAL_INTERPOLATOR.getInterpolation(interpolatedTime / 0.5f) * 0.79f) + MIN_PROGRESS_ARC + startTrim2;
            } else {
                startTrim = ring.getStartingStartTrim() + 0.79f;
                scaledTime = startTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation((interpolatedTime - 0.5f) / 0.5f)) * 0.79f) + MIN_PROGRESS_ARC);
            }
