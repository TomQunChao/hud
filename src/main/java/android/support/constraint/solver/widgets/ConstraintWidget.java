...
public class ConstraintWidget {
    ...
    /* JADX INFO: Multiple debug info for r0v7 'beginTarget'  android.support.constraint.solver.SolverVariable: [D('applyBoundsCheck' boolean), D('beginTarget' android.support.constraint.solver.SolverVariable)] */
    /* JADX INFO: Multiple debug info for r0v9 'beginTarget'  android.support.constraint.solver.SolverVariable: [D('applyBoundsCheck' boolean), D('beginTarget' android.support.constraint.solver.SolverVariable)] */
    private void applyConstraints(LinearSystem system, boolean parentWrapContent, SolverVariable parentMin, SolverVariable parentMax, DimensionBehaviour dimensionBehaviour, boolean wrapContent, ConstraintAnchor beginAnchor, ConstraintAnchor endAnchor, int beginPosition, int dimension, int minDimension, int maxDimension, float bias, boolean useRatio, boolean inChain, int matchConstraintDefault, int matchMinDimension, int matchMaxDimension, float matchPercentDimension, boolean applyPosition) {
        ...
        boolean matchConstraintDefault6;
        ...
        if (!variableSize2) {
            if (wrapContent) {
                ...
                if (maxDimension < Integer.MAX_VALUE) {
                    system.addLowerThan(end2, begin5, maxDimension, i7);
                }
            } else {
            ...
            if (matchMinDimension4 > 0) {
                i5 = 6;
                system.addGreaterThan(end2, begin5, matchMinDimension4, 6);
                dimension2 = Math.max(dimension2, matchMinDimension4);
            } else {
                i5 = 6;
            }
            if (matchMaxDimension3 > 0) {
                system.addLowerThan(end2, begin5, matchMaxDimension3, i5);
        ...
        if (!applyPosition) {
                    ...
                    if (!isBeginConnected && isEndConnected) {
                        ...
                        if (variableSize2) {
                            if (parentWrapContent && minDimension == 0) {
                                system.addGreaterThan(end2, begin5, 0, 6);
                            }
                            if (matchConstraintDefault3 == 0) {
                                ...
                                matchConstraintDefault6 = applyCentering;
                            } else {
                                beginTarget2 = beginTarget;
                        ...
                        if (matchConstraintDefault6) {
                            endTarget2 = endTarget;
                            end = end2;
                            begin4 = begin5;
                            system.addCentering(begin5, beginTarget2, beginAnchor.getMargin(), bias, endTarget, end2, endAnchor.getMargin(), centeringStrength);
                            i4 = 0;
                        ...
                        if (parentWrapContent) {
                            i3 = 6;
                            system.addGreaterThan(begin4, parentMin, i4, 6);
                        } else {
                            i3 = 6;
                        }
                    }
            ...
            if (parentWrapContent) {
                system.addGreaterThan(parentMax, begin3, i4, i3);
                return;
            }
            return;
        }
        if (matchConstraintDefault4 < i && parentWrapContent) {
            system.addGreaterThan(begin, parentMin, i2, 6);
            system.addGreaterThan(parentMax, begin2, i2, 6);
        }
    }
